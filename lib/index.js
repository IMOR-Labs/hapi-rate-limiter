...
exports.register = (server, options) => {
  server.ext('onPreStart', async () => {
    sha = await options.redisClient.script('LOAD', SCRIPT);
  });
  ...
  server.ext('onPostAuth', async (request, h) => {
    ...
    try {
      const [count, ttl] = await options.redisClient.evalsha(sha, 1, key, rate.window);
      ...
      if (remaining < 0) {
        return options.overLimitError(rate, request);
      }
